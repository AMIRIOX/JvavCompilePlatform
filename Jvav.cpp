...
main:
    ...
    if (command == "help") {
        ...
        if(compile_result==STATUS_SUCCESS){
            ...
            cout << "\nCompiler error or there is no g++ compiler environment!\n";
        }else if(compile_result == STATUS_NO_IDEN){
            cout << "\nThe identifier file is missing, please check if the identity.res file is in good condition.\n";
...
cn_main:
    ...
    if (cn_command == "帮助") {
        ...
        cout << "Jvav编程器,和Jvav编译平台，版本为2."
                "2预览版1。\n这只是一个梗，但是我们依然把它变成了现实。最早是张"
                "浩洋提出的这个梗。\n该版本支持脱离jdk的运行。但需要g++环境。\n该程序编写者为3"
                "0266。\n";
        goto cn_main;
...
upgrade:
    ...
    if (ret == -1) {
        if (language == "English") {
            cout
                << "Jvav>upgrade>Error! The update failed! Because the version "
                   "list cannot be obtained on the authentication server!\n";
            goto main;
        } else if (language == "简体中文") {
        ...
        if (language == "English") {
            cout
                << "Jvav>upgrade>Updated versionavailable for upgrades, please "
                   "go to https://30266-official.github.io/updates/Jvav.zip "
                   "download!\n";
            goto main;
        } else if (language == "简体中文") {
        ...
        if (language == "English") {
            cout << "Jvav>upgrade>Error!We can't get a version list because "
                    "your version is coming from a future version, so we don't "
                    "support it.\n";
            goto main;
            cout << ret;
        } else if (language == "简体中文") {
            cout << "Jvav>更新>"
                    "错误！我们无法获取版本列表，因为您的版本是来自未来的版本，"
                    "故我们不提供支持。\n";
            goto cn_main;
        }
    }
